    ...
    <div className="min-h-screen bg-background p-6 md:p-10 space-y-10 max-w-7xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground flex items-center gap-2 mt-1">
              {result.class_number} Class &bull; {result.language.toUpperCase()} &bull; {result.student_city}
            </p>
          </div>
        </div>
        <div className="flex gap-4">
          <Card className="bg-primary/5 border-primary/20">
            <CardContent className="p-4 py-3 flex flex-col items-center">
              <span className="text-xs font-bold text-primary uppercase tracking-tighter">Overall Score</span>
              <span className="text-3xl font-bold font-headline">{result.percentage}%</span>
            </CardContent>
          </Card>
          <Card className="bg-accent/5 border-accent/20">
            <CardContent className="p-4 py-3 flex flex-col items-center">
              <span className="text-xs font-bold text-accent uppercase tracking-tighter">Points</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="border-border bg-secondary/10">
            <CardHeader>
              ...
              <CardDescription>Generative analysis of error patterns and behavioral dynamics.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {analysis ? (
                <>
                  <div className="p-4 rounded-xl bg-primary/5 border border-primary/10">
                    <h4 className="font-bold text-primary mb-2 flex items-center gap-2">
                      <BarChart2 className="w-4 h-4" /> Performance Summary
                    </h4>
                  ...
                  <div className="space-y-4">
                    <h4 className="font-bold flex items-center gap-2">
                      <BookOpen className="w-4 h-4 text-accent" /> Learning Pathways
                    </h4>
                ...
                <div className="text-center py-10 text-muted-foreground border-2 border-dashed rounded-xl">
                  AI Analysis has not been generated for this result yet.
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-headline font-bold flex items-center gap-2">
              <Clock className="w-5 h-5 text-muted-foreground" /> Question Timeline
            </h3>
                  ...
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs text-muted-foreground uppercase font-bold tracking-tighter">{a.subject}</span>
                        <span className="text-sm font-medium">Student: {a.student_answer || 'Skipped'} | Correct: {a.correct_answer}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="text-xs font-mono text-muted-foreground">{a.time_spent_seconds}s</span>
        ...
        <div className="space-y-8">
          <Card className="border-border bg-secondary/10">
            ...
            <CardContent className="space-y-6">
              <div className="text-center p-6 rounded-xl bg-secondary/50 border border-border">
                <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Total Violations</p>
              ...
              {logs.length > 0 && (
                <div className="space-y-3">
                  <p className="text-xs font-bold text-muted-foreground uppercase">Activity Logs</p>
                  {logs.map((log, i) => (
                    <div key={i} className="flex gap-3 text-sm p-3 rounded-lg bg-background border border-border">
                      <AlertCircle className="w-4 h-4 text-destructive shrink-0 mt-0.5" />
                      <div>
                        <p className="font-semibold text-xs">{log.event_type.replace('_', ' ').toUpperCase()}</p>
                        <p className="text-xs text-muted-foreground">{log.details}</p>
                        <p className="text-[10px] text-muted-foreground/60 mt-1">Q{log.question_number} &bull; {new Date(log.created_at).toLocaleTimeString()}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {analysis?.antiCheatBehaviorAnalysis && (
                <div className="p-3 rounded-lg bg-destructive/5 border border-destructive/10 text-xs italic">
                  <span className="font-bold block mb-1">AI Behavior Audit:</span>
                  {analysis.antiCheatBehaviorAnalysis}
                </div>
              )}
            </CardContent>
          ...
          <Card className="border-border bg-secondary/10">
            <CardHeader>
              <CardTitle className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Contact Details</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <p className="text-xs text-muted-foreground">Parent WhatsApp</p>
              ...
              <div>
                ...
                <p className="font-medium">
                  {result.completed_at ? 
                    `${Math.round((new Date(result.completed_at).getTime() - new Date(result.started_at).getTime()) / 60000)} minutes` : 
                    'Active'
                  }
                </p>
              </div>
